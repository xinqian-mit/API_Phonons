...
Supercell = Phonopy(Prim_cell,np.diag(Ncells)).supercell
...
Scells_phonopy = phonon_scell.supercells_with_displacements # This returns a list of Phononpy atoms object


force_set= PhonIO.parse_FORCE_SETS() # parse force_sets
phonon_scell.dataset = force_set # force_set is a list of forces and displacements
...
if NAC == True:
    nac_params = PhonIO.get_born_parameters(
            open("BORN"),
            phonon_scell.primitive,
            phonon_scell.primitive_symmetry)
...
phonon_scell.run_band_structure(bands, with_eigenvectors=True)
